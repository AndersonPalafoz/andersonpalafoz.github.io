...
import { BookOpen, Clock, Inbox, Loader2, StickyNote, Trash2 } from "lucide-react";
...
export function ProfileNotesSection() {
  ...
  async function deleteNote(note: LessonNote) {
    if (note.deletedByAdminAt || deletingId !== null) return;
    const confirmed = window.confirm(`Excluir a anotação da aula “${note.lessonTitle}”? Esta ação não pode ser desfeita.`);
    if (!confirmed) return;
