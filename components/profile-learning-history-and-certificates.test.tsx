...
const historyRouteSource = readFileSync(new URL("../app/api/user/historico/route.ts", import.meta.url), "utf8");
...
describe("Profile Learning History & Certificates Component", () => {
  it("contém títulos e seções exigidas para histórico e certificados", () => {
    ...
    expect(componentSource).toContain("/api/user/certificates");
    expect(componentSource).toContain("Baixar PDF Assinado");
    expect(componentSource).toContain("CertificateShareButton");
    expect(historyRouteSource).toContain("getUserEnrollments(dbUser.id)");
    expect(historyRouteSource).toContain("enrollments,");
  });
});
