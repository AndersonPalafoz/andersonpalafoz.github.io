...
import { AlertCircle, ArrowRight, CheckCircle2, FileSignature, Loader2, MessageSquare, ShieldCheck, Users } from "lucide-react";
        ...
        {items.map(item => {
          ...
          return (
            <Link key={item.key} href={item.href} className={`group rounded-2xl border p-4 transition hover:-translate-y-0.5 hover:shadow-md active:scale-[0.99] ${toneClasses}`}>
              <div className="flex items-start justify-between gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/80 shadow-sm dark:bg-black/10"><Icon size={17} /></span><span className="text-2xl font-black">{loading ? "—" : item.count}</span></div>
              <p className="mt-3 text-sm font-black">{item.label}</p>
              <p className="mt-1 text-[11px] leading-relaxed opacity-75">{item.description}</p>
              <span className="mt-3 inline-flex items-center gap-1 text-[11px] font-black">Abrir fila <ArrowRight size={13} className="transition-transform group-hover:translate-x-0.5" /></span>
            </Link>
          );
        })}
      </div>
