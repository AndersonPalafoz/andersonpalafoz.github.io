...
export function HomepageExperience({ lessonCount }: { lessonCount: number }) {
  ...
  const closeCookies = (value: "accepted" | "dismissed") => {
    document.cookie = `site-cookie-consent=${value}; max-age=31536000; path=/; SameSite=Lax`;
  ...
  return <main className="homepage bg-white text-slate-900 dark:bg-slate-950 dark:text-slate-100">
    <section className="home-hero relative overflow-hidden bg-white dark:bg-slate-950">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_75%_20%,rgba(214,40,40,0.08),transparent_34%),linear-gradient(180deg,#ffffff_0%,#f8fafc_100%)] dark:bg-[radial-gradient(circle_at_75%_20%,rgba(214,40,40,0.12),transparent_34%),linear-gradient(180deg,#020617_0%,#0f172a_100%)]" />
      <div className="relative mx-auto grid min-h-0 lg:min-h-[min(780px,calc(100vh-76px))] max-w-7xl items-center gap-10 px-5 pb-16 pt-12 sm:px-8 lg:grid-cols-[1.02fr_.98fr] lg:gap-16 lg:px-12 lg:py-20">
        <Reveal className="relative z-10 max-w-2xl">
          ...
          <h1 aria-live="polite" className="max-w-2xl text-4xl font-black leading-[1.03] tracking-[-0.04em] text-slate-900 dark:text-white sm:text-6xl lg:text-7xl">{slide.title}</h1>
          ...
          <div className="mt-10 flex flex-wrap items-center gap-3" role="group" aria-label="Controles da apresentação"><button type="button" onClick={() => goToSlide(-1)} aria-label="Slide anterior" className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-slate-200 text-slate-600 transition hover:border-red-300 hover:text-red-700 dark:border-slate-700 dark:text-slate-300"><ChevronLeft size={16} /></button><div className="flex gap-2" aria-label="Slides da apresentação">{slides.map((item, index) => <button key={item.title} type="button" aria-label={`Ir para slide ${index + 1}`} aria-current={index === activeSlide} onClick={() => setActiveSlide(index)} className={`h-2 rounded-full transition-all ${index === activeSlide ? "w-10 bg-red-600" : "w-2 bg-red-200 hover:bg-red-400"}`} />)}</div><button type="button" onClick={() => goToSlide(1)} aria-label="Próximo slide" className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-slate-200 text-slate-600 transition hover:border-red-300 hover:text-red-700 dark:border-slate-700 dark:text-slate-300"><ChevronRight size={16} /></button><span className="text-xs font-bold text-slate-500">{String(activeSlide + 1).padStart(2, "0")} / 03</span><button type="button" onClick={() => setIsPaused((paused) => !paused)} aria-pressed={isPaused} className="text-xs font-black text-slate-500 underline-offset-4 hover:text-red-700 hover:underline">{isPaused ? "Continuar" : "Pausar"}</button></div>
        </Reveal>
        <Reveal className="relative mx-auto mt-2 w-full max-w-[560px] lg:mt-0 lg:justify-self-end">
          <div className="absolute -right-5 -top-7 hidden h-28 w-28 rounded-full border-[18px] border-slate-300 sm:block" /><div className="relative overflow-hidden rounded-[2rem] border-[10px] border-white bg-slate-200 dark:border-slate-800 dark:bg-slate-900 shadow-2xl shadow-slate-950/15"><Image src="/principal.png" alt="Anderson Palafoz, professor de inglês" width={800} height={800} priority className="aspect-[4/4.5] w-full object-cover object-[center_25%]" /><div className="absolute bottom-4 left-4 right-4 flex items-center justify-between rounded-2xl bg-white/95 p-4 shadow-lg backdrop-blur dark:bg-slate-950/90"><div><p className="text-xs font-black uppercase tracking-wider text-red-600 dark:text-red-300">Plataforma ativa</p><p className="mt-1 text-sm font-bold text-slate-900 dark:text-slate-100">Aprenda no seu ritmo</p></div><div className="flex h-10 w-10 items-center justify-center rounded-full bg-red-600 text-white"><Play size={16} fill="currentColor" /></div></div></div>
        </Reveal>
      </div>
    </section>
    ...
    <Reveal><section className="mx-5 overflow-hidden rounded-[2rem] bg-slate-900 sm:mx-8 lg:mx-auto lg:max-w-7xl"><div className="grid items-center gap-8 px-7 py-12 sm:px-12 lg:grid-cols-[1fr_auto] lg:px-16"><div><p className="mb-3 text-xs font-black uppercase tracking-[0.18em] text-red-300">Comece pelo próximo passo</p><h2 className="max-w-2xl text-3xl font-black tracking-tight text-white sm:text-4xl">Seu inglês merece uma experiência feita com clareza.</h2><p className="mt-4 max-w-xl text-sm leading-7 text-slate-300">Explore os cursos, encontre materiais gratuitos e fale comigo sobre seus objetivos.</p></div><Link href="/contato" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-white px-6 text-sm font-black text-slate-900 transition hover:-translate-y-1 hover:bg-red-50">Vamos conversar <ArrowRight size={17} /></Link></div></section></Reveal>
    <div className="pb-10 pt-20"><GoogleReviewsSection compact limit={3} /></div>

    {cookieVisible && <aside className="fixed inset-x-3 bottom-[max(0.75rem,env(safe-area-inset-bottom))] z-50 mx-auto max-w-xl rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl dark:border-slate-700 dark:bg-slate-900 sm:flex sm:items-center sm:gap-5 sm:p-5" role="dialog" aria-label="Aviso de cookies"><div className="min-w-0 flex-1"><p className="text-sm font-black text-slate-900 dark:text-white">Sua privacidade importa</p><p className="mt-1 text-xs leading-5 text-slate-600 dark:text-slate-300">Usamos cookies essenciais para manter o site funcionando e melhorar sua experiência.</p></div><div className="mt-3 flex shrink-0 items-center gap-2 sm:mt-0"><Link href="/privacidade" className="rounded-full px-3 py-2 text-xs font-bold text-red-600 hover:bg-red-50">Saiba mais</Link><button type="button" aria-label="Fechar aviso de cookies" onClick={() => closeCookies("dismissed")} className="rounded-full p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-900 dark:hover:bg-slate-800 dark:hover:text-white" title="Fechar"><X size={15} /></button><button type="button" onClick={() => closeCookies("accepted")} className="rounded-full bg-red-600 px-4 py-2 text-xs font-black text-white hover:bg-red-700">Aceitar</button></div></aside>}
  </main>;
}
