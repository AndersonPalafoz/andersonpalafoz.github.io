...
export function HomepageExperience({ lessonCount }: { lessonCount: number }) {
  ...
  return <main className="homepage bg-white text-[#333]">
    <section className="home-hero relative overflow-hidden bg-white">
      ...
      <div className="relative mx-auto grid min-h-[min(780px,calc(100vh-76px))] max-w-7xl items-center gap-10 px-5 pb-16 pt-12 sm:px-8 lg:grid-cols-[1.02fr_.98fr] lg:gap-16 lg:px-12 lg:py-20">
        <Reveal className="relative z-10 max-w-2xl">
          ...
          <h1 className="max-w-2xl text-5xl font-black leading-[1.03] tracking-[-0.04em] text-slate-900 sm:text-6xl lg:text-7xl">{slide.title}</h1>
          <p className="mt-6 max-w-xl text-base leading-8 text-slate-600 sm:text-lg">{slide.text}</p>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link href={slide.href} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-[#d62828] px-6 text-sm font-black text-white shadow-lg shadow-red-700/20 transition hover:-translate-y-1 hover:bg-[#b91c1c]">{slide.cta}<ArrowRight size={17} /></Link><Link href="/sobre" className="inline-flex min-h-12 items-center justify-center rounded-full border border-slate-300 bg-white/70 px-6 text-sm font-black text-[#4d4140] transition hover:-translate-y-1 hover:border-red-300 hover:text-red-700">Conhecer minha abordagem</Link></div>
    ...
    {cookieVisible && <aside className="fixed inset-x-4 bottom-4 z-50 mx-auto max-w-2xl rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl sm:flex sm:items-center sm:gap-5 sm:p-5" role="dialog" aria-label="Aviso de cookies"><div className="min-w-0 flex-1"><p className="text-sm font-black text-slate-900">Sua privacidade importa</p><p className="mt-1 text-xs leading-5 text-slate-600">Usamos cookies essenciais para manter o site funcionando e melhorar sua experiência.</p></div><div className="mt-3 flex shrink-0 items-center gap-2 sm:mt-0"><Link href="/privacidade" className="rounded-full px-3 py-2 text-xs font-bold text-red-600 hover:bg-red-50">Saiba mais</Link><button type="button" onClick={acceptCookies} className="rounded-full bg-red-600 px-4 py-2 text-xs font-black text-white hover:bg-red-700">Aceitar</button></div></aside>}
  </main>;
}
