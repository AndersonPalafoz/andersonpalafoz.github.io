...
export function AdminSearchWidget() {
  ...
  if (loading) {
    return (
      <div className="surface-card p-6 sm:p-8 flex items-center justify-center gap-3">
        <Loader2 className="animate-spin text-red-600" size={24} />
        <span className="text-xs sm:text-sm font-semibold text-muted-foreground">Carregando motor de busca administrativa...</span>
      </div>
    );
  }
    ...
    <div className="surface-card p-4 sm:p-8 space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <h2 className="text-lg sm:text-xl font-bold text-foreground flex items-center gap-2">
            <Search className="text-red-600 shrink-0" size={20} /> Motor de Busca Ampliado & Mobile
          </h2>
          <p className="text-xs text-muted-foreground mt-1">
            Pesquise por professores, alunos e cursos com transições fluidas e otimização para celular.
          </p>
        </div>
        <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-2">
          <button
            onClick={() => handleCategoryChange("all")}
            className={`px-3 py-2 sm:py-1.5 rounded-xl text-xs font-bold transition text-center ${filterCategory === "all" ? "bg-red-600 text-white shadow-md" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}
          >
            Todos
          </button>
          <button
            onClick={() => handleCategoryChange("teachers")}
            className={`px-3 py-2 sm:py-1.5 rounded-xl text-xs font-bold transition text-center ${filterCategory === "teachers" ? "bg-red-600 text-white shadow-md" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}
          >
            Professores ({teachers.length})
          </button>
          <button
            onClick={() => handleCategoryChange("students")}
            className={`px-3 py-2 sm:py-1.5 rounded-xl text-xs font-bold transition text-center ${filterCategory === "students" ? "bg-red-600 text-white shadow-md" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}
          >
            Alunos ({students.length})
          </button>
          <button
            onClick={() => handleCategoryChange("courses")}
            className={`px-3 py-2 sm:py-1.5 rounded-xl text-xs font-bold transition text-center ${filterCategory === "courses" ? "bg-red-600 text-white shadow-md" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}
          >
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Busque por nome, email ou título do curso..."
          className="w-full rounded-2xl border border-border bg-background pl-11 pr-10 py-3.5 text-xs sm:text-sm font-semibold text-foreground focus:outline-none focus:ring-2 focus:ring-red-600 shadow-sm"
        />
        {query && (
          <button
            onClick={() => setQuery("")}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 relative">
        ...
        {filterCategory !== "students" && filterCategory !== "courses" && (
          <div className="space-y-3 flex flex-col justify-between p-4 rounded-2xl border border-border/60 bg-muted/20">
            <div>
              ...
              {isPending ? (
                <div className="space-y-2">
                  {[1, 2, 3].map((n) => (
                    <div key={n} className="p-3 rounded-xl border border-border/70 bg-muted/20 animate-pulse flex items-center justify-between">
                ...
                <p className="text-xs text-muted-foreground p-4 bg-muted/30 rounded-xl border border-border/50 text-center">Nenhum professor encontrado.</p>
              ) : (
                <div className="space-y-2">
                  {paginatedTeachers.map((t) => (
                    <div key={t.id} className="p-3.5 rounded-xl border border-border/70 bg-background flex items-center justify-between hover:border-red-300 transition shadow-xs">
                      <div className="min-w-0 pr-2">
                        <h4 className="text-xs font-black text-foreground truncate">{t.name || "Professor"}</h4>
                        <p className="text-[11px] text-muted-foreground mt-0.5 truncate">{t.email}</p>
                      </div>
                      <Link
                        href="/admin/usuarios"
                        className="px-2.5 py-1 rounded-lg bg-amber-500/10 text-amber-600 hover:bg-amber-500 hover:text-white transition text-[11px] font-bold shrink-0"
                      >
                        Gerenciar
                      </Link>
                    </div>
          ...
          <div className="space-y-3 flex flex-col justify-between p-4 rounded-2xl border border-border/60 bg-muted/20">
            <div>
              ...
              {isPending ? (
                ...
                <p className="text-xs text-muted-foreground p-4 bg-muted/30 rounded-xl border border-border/50 text-center">Nenhum aluno encontrado.</p>
              ) : (
                <div className="space-y-2">
                  {paginatedStudents.map((s) => (
                    <div key={s.id} className="p-3.5 rounded-xl border border-border/70 bg-background flex items-center justify-between hover:border-red-300 transition shadow-xs">
                      <div className="min-w-0 pr-2">
                        <h4 className="text-xs font-black text-foreground truncate">{s.name || "Aluno"}</h4>
                        <p className="text-[11px] text-muted-foreground mt-0.5 truncate">{s.email}</p>
                      </div>
                      <Link
                        href="/admin/usuarios"
                        className="px-2.5 py-1 rounded-lg bg-red-600/10 text-red-600 hover:bg-red-600 hover:text-white transition text-[11px] font-bold shrink-0"
                      >
                        Gerenciar
                      </Link>
                    </div>
          ...
          <div className="space-y-3 flex flex-col justify-between p-4 rounded-2xl border border-border/60 bg-muted/20">
            <div>
              ...
              {isPending ? (
                ...
                <p className="text-xs text-muted-foreground p-4 bg-muted/30 rounded-xl border border-border/50 text-center">Nenhum curso encontrado.</p>
              ) : (
                <div className="space-y-2">
                  {paginatedCourses.map((c) => (
                    <div key={c.id} className="p-3.5 rounded-xl border border-border/70 bg-background flex items-center justify-between hover:border-red-300 transition shadow-xs">
                      <div className="min-w-0 pr-2">
                        <h4 className="text-xs font-black text-foreground truncate">{c.title}</h4>
                        <p className="text-[11px] text-muted-foreground mt-0.5">Nível {c.level}</p>
                      </div>
                      <Link
                        href={`/cursos/${c.id}`}
                        className="px-2.5 py-1 rounded-lg bg-red-600/10 text-red-600 hover:bg-red-600 hover:text-white transition text-[11px] font-bold flex items-center gap-1 shrink-0"
                      >
