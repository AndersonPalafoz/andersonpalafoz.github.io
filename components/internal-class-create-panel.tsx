...
export function InternalClassCreatePanel({ courses }: { courses: CourseOption[] }) {
  ...
  const usingNewCourse = values.courseMode === "new";
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    if (!usingNewCourse && !values.courseId) return setError("Selecione o curso que será a base desta turma ou crie um novo.");
    if (usingNewCourse && values.newCourseTitle.trim().length < 3) return setError("Informe o título do novo curso.");
    if (usingNewCourse && !values.newCourseLevel) return setError("Informe o nível do novo curso.");
    if (!values.offerName.trim()) return setError("Informe um nome para a turma.");
    if (!values.startDate || !values.endDate) return setError("Informe as datas de início e término da turma.");
    if (values.endDate < values.startDate) return setError("A data de término deve ser posterior ao início.");
    if (!values.classDays) return setError("Selecione pelo menos um dia da semana.");
    if (values.endTime <= values.startTime) return setError("O horário final deve ser posterior ao horário inicial.");
    setIsSubmitting(true);
    try {
      let courseId = Number(values.courseId);
      if (usingNewCourse) {
        const courseResponse = await fetch("/api/courses", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: values.newCourseTitle, level: values.newCourseLevel, category: values.newCourseCategory, description: values.newCourseDescription }) });
        const coursePayload = await courseResponse.json();
        if (!courseResponse.ok) throw new Error(coursePayload.error || "Não foi possível criar o curso.");
      ...
      const response = await fetch("/api/course-offers", {
        ...
        body: JSON.stringify({
          courseId, offerName: values.offerName.trim(), academicTerm: values.academicTerm.trim(), modality: values.modality,
          classDays: values.classDays.trim() || null, classTime: `${values.startTime} às ${values.endTime}`, startDate: values.startDate, endDate: values.endDate, workloadHours: Number(values.workloadHours || 40),
  ...
  return <section className="surface-card overflow-hidden border-red-200/80 bg-gradient-to-br from-red-50/80 via-card to-card p-0 shadow-sm dark:border-red-950/60 dark:from-red-950/20" aria-labelledby="create-internal-class-title">
    ...
    <form onSubmit={handleSubmit} className="grid gap-4 p-5 sm:p-7">
      <div className={sectionClass}><button type="button" onClick={() => toggleSection("course")} className="flex w-full items-center justify-between text-left"><span><span className="text-xs font-black uppercase tracking-[0.14em] text-red-600">01 · Base acadêmica</span><span className="mt-1 block text-base font-black text-foreground">Qual curso dará origem à turma?</span></span><ChevronDown className={`transition ${openSection === "course" ? "rotate-180" : ""}`} size={18} /></button>{openSection === "course" && <div className="mt-4 grid gap-4 sm:grid-cols-2"><div className="sm:col-span-2 grid gap-2 sm:grid-cols-2"><button type="button" onClick={() => update("courseMode", "existing")} className={`rounded-xl border px-4 py-3 text-left text-sm font-bold ${!usingNewCourse ? "border-red-600 bg-red-50 text-red-800 dark:bg-red-950/30 dark:text-red-100" : "border-border text-muted-foreground"}`}>Usar curso existente<span className="mt-1 block text-xs font-normal">Reutilizar conteúdo já cadastrado</span></button><button type="button" onClick={() => update("courseMode", "new")} className={`rounded-xl border px-4 py-3 text-left text-sm font-bold ${usingNewCourse ? "border-red-600 bg-red-50 text-red-800 dark:bg-red-950/30 dark:text-red-100" : "border-border text-muted-foreground"}`}>Criar curso novo<span className="mt-1 block text-xs font-normal">Começar a estrutura acadêmica agora</span></button></div>{!usingNewCourse ? <label className="text-sm font-bold text-foreground sm:col-span-2">Curso base<select name="courseId" value={values.courseId} onChange={(event) => update("courseId", event.target.value)} className={inputClass} required><option value="" disabled>{courses.length ? "Selecione um curso existente" : "Nenhum curso disponível"}</option>{courses.map((course) => <option key={course.id} value={course.id}>{course.title} · {course.level}</option>)}</select></label> : <div className="sm:col-span-2 grid gap-4 sm:grid-cols-2"><label className="text-sm font-bold text-foreground">Título do curso<input value={values.newCourseTitle} onChange={(event) => update("newCourseTitle", event.target.value)} maxLength={255} placeholder="Ex.: Inglês para viagens" className={inputClass} /></label><label className="text-sm font-bold text-foreground">Nível<select value={values.newCourseLevel} onChange={(event) => update("newCourseLevel", event.target.value)} className={inputClass}><option value="">Selecione</option><option>A1</option><option>A2</option><option>B1</option><option>B2</option><option>C1</option><option>C2</option></select></label><label className="text-sm font-bold text-foreground">Categoria<input value={values.newCourseCategory} onChange={(event) => update("newCourseCategory", event.target.value)} placeholder="Ex.: Inglês geral" className={inputClass} /></label><label className="text-sm font-bold text-foreground">Descrição<textarea value={values.newCourseDescription} onChange={(event) => update("newCourseDescription", event.target.value)} rows={3} placeholder="Apresente o objetivo do curso" className={inputClass} /></label></div>}{selectedCourse && <p className="flex items-start gap-2 rounded-xl bg-red-50 px-3 py-2.5 text-xs leading-5 text-red-800 dark:bg-red-950/30 dark:text-red-200"><Info size={15} className="mt-0.5 shrink-0" />A turma usará o conteúdo e a estrutura de {selectedCourse.title}. A criação não copia alunos nem cria uma turma externa.</p>}</div>}</div>
      ...
      <div className={sectionClass}><button type="button" onClick={() => toggleSection("schedule")} className="flex w-full items-center justify-between text-left"><span><span className="text-xs font-black uppercase tracking-[0.14em] text-red-600">04 · Calendário real</span><span className="mt-1 block text-base font-black text-foreground">Quando os encontros acontecem?</span></span><ChevronDown className={`transition ${openSection === "schedule" ? "rotate-180" : ""}`} size={18} /></button>{openSection === "schedule" && <div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="text-sm font-bold text-foreground">Primeiro encontro<input name="startDate" type="date" value={values.startDate} onChange={(event) => update("startDate", event.target.value)} className={inputClass} required /></label><label className="text-sm font-bold text-foreground">Último encontro<input name="endDate" type="date" value={values.endDate} onChange={(event) => update("endDate", event.target.value)} className={inputClass} required /></label><fieldset className="sm:col-span-2"><legend className="text-sm font-bold text-foreground">Dias da semana</legend><div className="mt-2 flex flex-wrap gap-2">{[["1","Seg"],["2","Ter"],["3","Qua"],["4","Qui"],["5","Sex"],["6","Sáb"],["0","Dom"]].map(([value, label]) => { const selected = values.classDays.split(",").filter(Boolean).includes(value); return <button key={value} type="button" aria-pressed={selected} onClick={() => update("classDays", selected ? values.classDays.split(",").filter((day) => day && day !== value).join(",") : [...values.classDays.split(",").filter(Boolean), value].join(","))} className={`min-h-10 rounded-xl border px-3.5 text-sm font-bold transition ${selected ? "border-red-600 bg-red-600 text-white" : "border-border bg-background text-muted-foreground hover:border-red-300"}`}>{label}</button>; })}</div></fieldset><label className="text-sm font-bold text-foreground">Início<input name="startTime" type="time" value={values.startTime} onChange={(event) => update("startTime", event.target.value)} className={inputClass} required /></label><label className="text-sm font-bold text-foreground">Fim<input name="endTime" type="time" value={values.endTime} onChange={(event) => update("endTime", event.target.value)} className={inputClass} required /></label><div className="sm:col-span-2 rounded-xl border border-red-200 bg-red-50/70 px-3 py-3 text-sm text-red-900 dark:border-red-950/60 dark:bg-red-950/20 dark:text-red-100">{values.startDate && values.endDate && values.classDays ? `Encontros recorrentes de ${values.startDate} a ${values.endDate}, das ${values.startTime} às ${values.endTime}.` : "Defina intervalo, dias e horário para criar uma agenda real."}</div></div>}</div>
