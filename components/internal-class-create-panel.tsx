...
export function InternalClassCreatePanel({ courses }: { courses: CourseOption[] }) {
    ...
    try {
      const response = await fetch("/api/course-offers", {
        ...
        body: JSON.stringify({
          ...
          durationValue: Number(form.get("durationValue") || 1),
          durationUnit: form.get("durationUnit"),
          status: "draft",
        }),
      });
  ...
  return <section className="surface-card overflow-hidden border-red-200/80 bg-gradient-to-br from-red-50/80 via-card to-card p-4 shadow-sm dark:border-red-950/60 dark:from-red-950/20 sm:p-6" aria-labelledby="create-internal-class-title"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.14em] text-red-600">Novo registro interno</p><h2 id="create-internal-class-title" className="mt-1 text-xl font-black text-foreground">Criar turma dentro da plataforma</h2><p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">Escolha um curso existente e crie uma oferta interna. Esta ação não cria turma externa nem copia alunos.</p></div><button type="button" onClick={closePanel} aria-label="Fechar formulário de nova turma" className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl text-muted-foreground hover:bg-background hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500"><X size={18} aria-hidden="true" /></button></div>
    <form onSubmit={handleSubmit} className="mt-5 grid gap-4 sm:grid-cols-2">
      ...
      <label className="grid gap-1.5 text-sm font-bold text-foreground">Formato de duração<select name="durationType" defaultValue="calendar_period" className="min-h-11 rounded-xl border border-border bg-background px-3 py-2.5 font-normal outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/20"><option value="calendar_period">Período de calendário</option><option value="workload">Por carga horária</option></select></label>
      <label className="grid gap-1.5 text-sm font-bold text-foreground">Período<select name="durationUnit" defaultValue="monthly" className="min-h-11 rounded-xl border border-border bg-background px-3 py-2.5 font-normal outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/20"><option value="monthly">Mensal</option><option value="bimonthly">Bimestral</option><option value="quarterly">Trimestral</option><option value="semester">Semestral</option><option value="annual">Anual</option><option value="hours">Horas</option></select></label>
      <label className="grid gap-1.5 text-sm font-bold text-foreground">Quantidade<input name="durationValue" type="number" min="1" step="1" defaultValue="1" className="min-h-11 rounded-xl border border-border bg-background px-3 py-2.5 font-normal outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/20" /></label>
      <label className="grid gap-1.5 text-sm font-bold text-foreground">Carga horária<input name="workloadHours" type="number" min="1" max="1000" step="1" defaultValue="40" className="min-h-11 rounded-xl border border-border bg-background px-3 py-2.5 font-normal outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/20" /></label>
