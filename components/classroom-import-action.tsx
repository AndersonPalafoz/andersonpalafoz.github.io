...
export function ClassroomImportAction() {
  ...
  return (
    <section className="surface-card space-y-4 border-2 border-dashed border-amber-200 bg-amber-50/30 p-6 text-center dark:border-amber-900/50 dark:bg-amber-950/20 sm:p-8">
      ...
      {showDetails && <div className="mx-auto max-w-lg rounded-2xl border border-border bg-background p-4 text-left text-xs leading-5 text-muted-foreground"><p>O Google Classroom exige uma autorização específica para leitura de turmas, atividades, entregas e participantes. O login padrão não solicita esses escopos.</p><div className="mt-3 flex flex-wrap gap-3"><button type="button" onClick={() => void startClassroomAuthorization()} className="inline-flex items-center gap-2 font-bold text-primary hover:underline"><ExternalLink size={14} /> Conectar Google Classroom</button><button type="button" onClick={() => void importCourses()} disabled={syncing || importingCoursework} className="inline-flex items-center gap-2 rounded-lg bg-primary px-3 py-2 font-bold text-primary-foreground disabled:cursor-wait disabled:opacity-60">{syncing ? "Importando..." : "Importar cursos"}</button><button type="button" onClick={() => void importCoursework()} disabled={syncing || importingCoursework || importingSubmissions} className="inline-flex items-center gap-2 rounded-lg border border-primary px-3 py-2 font-bold text-primary disabled:cursor-wait disabled:opacity-60">{importingCoursework ? "Importando atividades..." : "Importar atividades"}</button><button type="button" onClick={() => void importSubmissions()} disabled={syncing || importingCoursework || importingSubmissions} className="inline-flex items-center gap-2 rounded-lg border border-primary px-3 py-2 font-bold text-primary disabled:cursor-wait disabled:opacity-60">{importingSubmissions ? "Importando entregas..." : "Importar entregas e notas"}</button></div>{result && <p className="mt-3 font-semibold text-emerald-700">{result}</p>}{error && <p className="mt-3 font-semibold text-destructive">{error}</p>}</div>}
    </section>
  );
}
