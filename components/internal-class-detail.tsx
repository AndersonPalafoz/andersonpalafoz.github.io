...
type Summary = { sessions: Session[]; activities: Array<{ id: number; title: string; description?: string | null; dueDate: string | Date | null }>; attendances: Array<{ id: number; sessionId: number; studentId: number; status: string; notes?: string | null }>; averageProgress: number; attendanceRate: number; completedActivities: number; totalActivities: number; passingAverage: number; studentMetrics: Array<{ studentId: number; average: number | null }> };
...
export function InternalClassDetail({ offer, course, students, role }: Props) {
  ...
  const studentMetric = new Map((summary?.studentMetrics ?? []).map((metric) => [metric.studentId, metric.average]));
  const performance = (studentId: number) => { const value = studentMetric.get(studentId); if (value == null) return { label: "Sem média", className: "bg-muted text-muted-foreground" }; const passing = summary?.passingAverage ?? 6; if (value < passing - 0.3) return { label: `Abaixo · ${value}`, className: "bg-red-100 text-red-700 dark:bg-red-950/40 dark:text-red-300" }; if (value <= passing + 0.3) return { label: `Próximo da média · ${value}`, className: "bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300" }; return { label: `Acima · ${value}`, className: "bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300" }; };
