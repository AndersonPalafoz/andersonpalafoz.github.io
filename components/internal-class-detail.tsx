...
export function InternalClassDetail({ offer, course, students, role }: Props) {
  ...
  useEffect(() => { if (!offer.id) return; const timer = window.setTimeout(() => { void fetch(`/api/course-offers/${offer.id}/academic-summary`).then((response) => response.ok ? response.json() : null).then(setSummary).catch(() => setSummary(null)); }, 0); return () => window.clearTimeout(timer); }, [offer.id]);
  ...
  return <div className="flex flex-col gap-5">
    ...
    {activeTab === "attendance" && offer.id && <section className="rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-6" role="tabpanel"><h2 className="text-xl font-black text-foreground">Presença</h2><InternalClassAttendancePanel offerId={offer.id} sessions={summary?.sessions ?? []} students={students} attendances={summary?.attendances ?? []} onChanged={refreshSummary} /></section>}
    {activeTab === "activities" && <section className="rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-6" role="tabpanel"><h2 className="text-xl font-black text-foreground">Atividades</h2><InternalClassActivitiesPanel activities={summary?.activities ?? []} onChanged={refreshSummary} /></section>}
    <section className={`rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-6 ${activeTab === "attendance" || activeTab === "activities" ? "hidden" : ""}`} role="tabpanel"><h2 className="text-xl font-black text-foreground">{tabs.find((tab) => tab.id === activeTab)?.label}</h2>{activeTab === "students" ? <div className="mt-5 divide-y divide-border">{students.length ? students.map((student) => <div key={student.id} className="flex items-center justify-between gap-4 py-3"><div><p className="font-bold text-foreground">{student.name || "Aluno sem nome"}</p><p className="text-sm text-muted-foreground">{student.email || "E-mail não informado"}</p></div><div className="flex items-center gap-2"><span className="rounded-full bg-muted px-2.5 py-1 text-xs font-semibold text-muted-foreground">Ativo</span><button type="button" onClick={() => void removeStudent(student.id)} disabled={removingStudentId === student.id} className="inline-flex items-center gap-1 rounded-lg border border-destructive/30 px-2 py-1 text-xs font-bold text-destructive hover:bg-destructive/10 disabled:opacity-60" aria-label={`Desvincular ${student.name || "aluno"}`}>{removingStudentId === student.id ? <Loader2 size={13} className="animate-spin" aria-hidden="true" /> : <Trash2 size={13} aria-hidden="true" />}Desvincular</button></div></div>) : <p className="py-8 text-sm text-muted-foreground">Nenhum aluno vinculado ainda.</p>}</div> : activeTab === "sessions" ? <div className="mt-5 flex flex-col gap-3">{summary?.sessions.length ? summary.sessions.map((session) => <div key={session.id} className="flex flex-col gap-3 rounded-xl border border-border p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold text-foreground">{session.title}</p><p className="text-sm text-muted-foreground">{new Date(session.scheduledAt).toLocaleDateString("pt-BR")} · {session.durationMinutes ?? 60} min · {session.status}</p></div><div className="flex items-center gap-2"><button type="button" onClick={() => setEditingSession(session)} className="inline-flex items-center gap-1 rounded-lg border border-border px-2 py-1 text-xs font-bold text-foreground hover:bg-muted"><Edit3 size={13} aria-hidden="true" />Editar</button><button type="button" onClick={() => void deleteSession(session)} disabled={deletingSessionId === session.id} className="inline-flex items-center gap-1 rounded-lg border border-destructive/30 px-2 py-1 text-xs font-bold text-destructive hover:bg-destructive/10 disabled:opacity-60"><Trash2 size={13} aria-hidden="true" />{deletingSessionId === session.id ? "Excluindo" : "Excluir"}</button></div></div>) : <p className="py-8 text-sm text-muted-foreground">Nenhuma sessão registrada.</p>}</div> : <><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">{activeTab === "overview" ? "Acompanhe a operação da turma com dados acadêmicos reais." : "Os registros desta seção são carregados a partir dos vínculos acadêmicos existentes."}</p><div className="mt-6 grid gap-3 sm:grid-cols-3">{metrics.map((metric) => <div key={metric.label} className="rounded-2xl border border-dashed border-border p-4"><p className="text-xs font-bold uppercase tracking-wide text-muted-foreground">{metric.label}</p><p className="mt-2 text-2xl font-black text-foreground">{metric.value}</p></div>)}</div><p className="mt-5 text-xs font-semibold text-muted-foreground">Acesso: {role === "admin" ? "administrador" : "professor responsável"}.</p></>}</section>
