...
type ClassData = { id: number; className: string; institution?: string | null; courseName?: string | null; academicTerm?: string | null; passingAverage?: number | string | null; students?: Student[]; attendance?: AttendanceRow[]; grades?: GradeRow[]; materials?: Array<{ id: number; title?: string | null; description?: string | null }>; stats?: { total: number; active: number; completed: number } };
...
export function ExternalClassDetail({ id }: { id: string }) {
  ...
  const passingAverage = Number(data?.passingAverage ?? 6);
  const gradePerformance = (grade: GradeRow) => {
    const score = Number(grade.score);
    const maxScore = Number(grade.maxScore);
    const normalized = maxScore > 0 ? (score / maxScore) * 10 : score;
    if (normalized < passingAverage - 0.3) return { label: "Abaixo da média", card: "border-red-200 bg-red-50/70 dark:border-red-950/50 dark:bg-red-950/20", score: "text-red-600 dark:text-red-300", badge: "bg-red-100 text-red-700 dark:bg-red-950/50 dark:text-red-300" };
    if (normalized <= passingAverage + 0.3) return { label: "Na média", card: "border-amber-200 bg-amber-50/70 dark:border-amber-950/50 dark:bg-amber-950/20", score: "text-amber-700 dark:text-amber-300", badge: "bg-amber-100 text-amber-800 dark:bg-amber-950/50 dark:text-amber-300" };
    return { label: "Acima da média", card: "border-emerald-200 bg-emerald-50/70 dark:border-emerald-950/50 dark:bg-emerald-950/20", score: "text-emerald-700 dark:text-emerald-300", badge: "bg-emerald-100 text-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-300" };
  };
  ...
  return <section className="space-y-5">
    ...
    <div className="overflow-hidden rounded-2xl border border-border bg-card"><div className="flex overflow-x-auto border-b border-border">{tabs.map(({ id: tabId, label, count, icon: Icon }) => <button key={tabId} onClick={() => setTab(tabId)} className={`flex min-w-fit items-center gap-2 border-b-2 px-4 py-3 text-sm font-bold ${tab === tabId ? "border-red-600 text-red-600" : "border-transparent text-muted-foreground hover:bg-muted"}`}><Icon size={16} /> {label} <span className="rounded-full bg-muted px-2 py-0.5 text-xs">{count}</span></button>)}</div>
      <div className="p-5">
        ...
        {tab === "grades" && <div className="space-y-5"><div className="flex flex-wrap items-center gap-2 rounded-xl border border-border bg-muted/30 p-3 text-xs font-bold"><span>Média mínima: {passingAverage.toFixed(1)}</span><span className="rounded-full bg-red-100 px-2.5 py-1 text-red-700 dark:bg-red-950/50 dark:text-red-300">Abaixo da média</span><span className="rounded-full bg-amber-100 px-2.5 py-1 text-amber-800 dark:bg-amber-950/50 dark:text-amber-300">Na média (±0,3)</span><span className="rounded-full bg-emerald-100 px-2.5 py-1 text-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-300">Acima da média</span></div>{grades.length ? gradeStudents.map((student) => { const records = gradesByStudent.get(student.id) || []; if (!records.length) return null; return <div key={student.id} className="rounded-xl border border-border p-4"><div className="mb-3 flex items-center justify-between gap-3"><div><p className="font-black">{student.name}</p><p className="text-xs text-muted-foreground">{records.length} lançamento(s)</p></div><span className="rounded-full bg-blue-100 px-3 py-1 text-xs font-bold text-blue-700">SIMAL</span></div><div className="grid gap-2 sm:grid-cols-2">{records.map((grade) => <div key={grade.id} className={`rounded-lg border p-3 ${gradePerformance(grade).card}`}><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm font-bold">{grade.assessmentTitle}</p><div className="flex items-center gap-2"><span className={`rounded-full px-2 py-0.5 text-[11px] font-black ${gradePerformance(grade).badge}`}>{gradePerformance(grade).label}</span><p className={`text-sm font-black ${gradePerformance(grade).score}`}>{grade.score}/{grade.maxScore}</p></div></div><p className="mt-1 text-xs text-muted-foreground">{gradeComponentLabel(grade.assessmentComponent)}{grade.assessmentVersion ? ` · Versão ${grade.assessmentVersion}` : ""}{grade.unitNumber ? ` · Unidade ${grade.unitNumber}` : ""}{grade.assessmentDate ? ` · ${new Date(`${grade.assessmentDate}T12:00:00`).toLocaleDateString("pt-BR")}` : ""}</p>{editingGrade?.id === grade.id ? <div className="mt-3 grid gap-2 sm:grid-cols-2"><input value={gradeDraft.title} onChange={(event) => setGradeDraft((current) => ({ ...current, title: event.target.value }))} className="rounded-lg border border-border bg-background px-3 py-2 text-sm" aria-label="Título da nota" /><input value={gradeDraft.score} onChange={(event) => setGradeDraft((current) => ({ ...current, score: event.target.value }))} className="rounded-lg border border-border bg-background px-3 py-2 text-sm" aria-label="Nota" /><div className="flex gap-2 sm:col-span-2"><button type="button" onClick={() => void saveGradeEdit()} className="rounded-lg bg-red-600 px-3 py-2 text-xs font-bold text-white">Salvar alteração</button><button type="button" onClick={cancelGradeEdit} className="rounded-lg border border-border px-3 py-2 text-xs font-bold hover:bg-background">Cancelar / reverter</button></div></div> : <button type="button" onClick={() => editGrade(grade)} className="mt-3 rounded-lg border border-border px-2.5 py-1 text-xs font-bold hover:bg-background">Editar nota</button>}</div>)}</div></div>; }) : <p className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">Nenhuma nota SIMAL encontrada para esta turma.</p>}<ExternalAcademicEditor classId={data.id} students={students} onSaved={() => void load()} /></div>}
