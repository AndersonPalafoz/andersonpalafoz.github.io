...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      requiresBrandingDecision:
        "requiresBrandingDecision" in result
          ? result.requiresBrandingDecision
          : false,
      ...
      issueDate:
        result.certificate?.issuedAt?.toLocaleDateString("pt-BR") || null,
      certificateCode: result.certificate?.certificateCode || null,
      certificateUrl: result.certificate?.certificateUrl || null,
      signedCertificateDownloadUrl: result.certificate?.signedPdfUrl
        ? `/api/certificates/${result.certificate.id}/download`
        : null,
    ...
    return NextResponse.json(
      { error: "Não foi possível emitir o certificado agora." },
      { status: 500 }
    );
  }
}
