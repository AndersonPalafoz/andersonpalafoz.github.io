...
export async function GET() {
  try {
    ...
    return NextResponse.json({ exports: exportsList }, { headers: { "Cache-Control": "private, no-store" } });
  } catch (error) {
    console.error("Error fetching teacher ZIP export history:", error);
    return NextResponse.json({ error: "Não foi possível carregar o histórico de exportações." }, { status: 500 });
  }
}
