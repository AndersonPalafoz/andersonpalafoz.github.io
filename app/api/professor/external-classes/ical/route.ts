...
export async function GET() {
  try {
    ...
    for (const item of classes) {
      const summary = encodeURIComponent(`${item.title} (${item.organizationName})`);
      const description = encodeURIComponent(`Modalidade: ${item.modality}. Dias: ${item.classDays}. Horário: ${item.classTime}. Local/Link: ${item.locationOrLink}`);
      icsContent += "BEGIN:VEVENT\n";
      icsContent += `UID:external-class-${item.id}@andersonpalafoz.vercel.app\n`;
      icsContent += `SUMMARY:${item.title} - ${item.organizationName}\n`;
      icsContent += `DESCRIPTION:Modalidade: ${item.modality} \\n Dias: ${item.classDays} \\n Horario: ${item.classTime} \\n Local: ${item.locationOrLink}\n`;
      icsContent += `LOCATION:${item.locationOrLink || "Online"}\n`;
