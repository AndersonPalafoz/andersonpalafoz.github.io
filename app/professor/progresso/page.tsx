...
import { users, progress, enrollments } from "@/drizzle/schema";
import { and, eq, inArray, isNull } from "drizzle-orm";
import { canAccessProfessorPortal, getEffectiveRole, roleLabel } from "@/lib/role-capabilities";
...
export default async function TeacherStudentProgressPage() {
  ...
  if (!session?.user || !canAccessProfessorPortal({ email, role: session.user.role })) redirect("/");
  const visibleCourses = await getTeacherCourses(email);
  const effectiveRole = getEffectiveRole({ email, role: session.user.role });
