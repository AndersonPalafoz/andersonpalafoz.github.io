    ...
    <main className="min-h-screen bg-gray-50 px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-5xl space-y-6">
        <header className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-bold text-gray-900">Solicitações de alunos</h1>
              <p className="mt-2 max-w-2xl text-gray-600">Revise solicitações de acesso de alunos. Professores não podem aprovar outros professores nem alterar papéis administrativos.</p>
            </div>
        ...
        <section className="rounded-2xl border border-gray-200 bg-white shadow-sm">
          {loading ? (
            <div className="flex items-center justify-center gap-3 p-12 text-gray-600"><Loader2 className="animate-spin text-red-600" /> Carregando solicitações...</div>
          ) : students.length === 0 ? (
            <div className="p-12 text-center"><UserCheck size={44} className="mx-auto text-green-600" /><h2 className="mt-4 text-xl font-bold text-gray-900">Nenhuma solicitação pendente</h2><p className="mt-2 text-gray-600">Novas solicitações aparecerão aqui para revisão.</p></div>
          ) : (
            <div className="divide-y divide-gray-100">
              {students.map((student) => (
                <article key={student.id} className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
                  <div>
                    <h2 className="font-bold text-gray-900">{student.name || "Usuário sem nome"}</h2>
                    <p className="text-sm text-gray-600">{student.email || "Email não informado"}</p>
                    <p className="mt-1 text-xs text-gray-500">Solicitado em {new Date(student.createdAt).toLocaleDateString("pt-BR")}</p>
                  </div>
                  <div className="flex flex-col gap-2 sm:flex-row">
                    <button disabled={actionId === student.id} onClick={() => void review(student.id, "reject")} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-red-200 px-4 py-2 text-sm font-semibold text-red-700 hover:bg-red-50 disabled:opacity-60"><UserX size={16} /> Recusar</button>
