...
import { Bold, ChevronLeft, Heading2, Italic, List, Loader2, Save, Strikethrough, Underline } from "lucide-react";
  ...
  const handleSave = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/admin/activities", {
        ...
        body: JSON.stringify({ courseId: Number(courseId), title, description, type, dueDate: dueDate || undefined }),
      });
    ...
    <div className="min-h-screen bg-gray-50 pb-12">
      <header className="border-b border-gray-200 bg-white">
        <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
          <Link href="/admin" className="mb-3 inline-flex items-center gap-1 text-sm font-semibold text-gray-500 hover:text-red-600"><ChevronLeft size={16} /> Painel administrativo</Link>
          <h1 className="text-3xl font-black tracking-tight text-gray-950">Provas e atividades</h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-gray-500">Crie questões com formatação visual, associe-as a um curso e organize avaliações com prazo.</p>
        </div>
      </header>
      ...
      <main className="mx-auto grid max-w-7xl gap-6 px-4 py-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.85fr)] sm:px-6">
        <form onSubmit={handleSave} className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="space-y-4">
            ...
            <div><div className="mb-2 flex flex-wrap items-center gap-1 rounded-t-xl border border-b-0 border-gray-300 bg-gray-50 p-2"><button type="button" onClick={applyHeading} aria-label="Título da questão" title="Título da questão" className="rounded-lg p-2 text-gray-600 hover:bg-white hover:text-red-600"><Heading2 size={17} /></button>{toolbarItems.map(({ command, label, icon: Icon }) => <button key={command} type="button" onClick={() => applyCommand(command)} aria-label={label} title={label} className="rounded-lg p-2 text-gray-600 hover:bg-white hover:text-red-600"><Icon size={17} /></button>)}</div><div ref={editorRef} contentEditable suppressContentEditableWarning role="textbox" aria-label="Enunciado formatado da questão" data-placeholder="Escreva o enunciado da questão, inclua exemplos, instruções e alternativas..." className="min-h-56 rounded-b-xl border border-gray-300 p-4 text-sm leading-7 outline-none focus:border-red-500 focus:ring-2 focus:ring-red-100 [&:empty]:before:text-gray-400 [&:empty]:before:content-[attr(data-placeholder)]" /></div>
            <Button type="submit" disabled={saving} className="h-11 w-full rounded-xl bg-red-600 font-bold text-white hover:bg-red-700">{saving ? <Loader2 size={17} className="mr-2 animate-spin" /> : <Save size={17} className="mr-2" />} Salvar prova ou atividade</Button>
          </div>
        </form>
