...
export default async function DashboardPage() {
  ...
  let externalAcademic: Array<{ className: string; courseName: string; hasUnits: boolean; unitCount: number; gradingScope: string; passingAverage: number; status: ReturnType<typeof getAcademicStatus>; attendanceRate: number | null; absent: number; totalClasses: number; gradeCount: number; units: Array<{ number: number; average: number | null; minimum: number; ratio: number }> }> = [];
    ...
    try {
      ...
      if (account?.email) {
        ...
        if (classIds.length > 0) {
          ...
          const attendanceRows = await db.query.externalClassAttendance.findMany({ where: inArray(externalClassAttendance.externalClassId, classIds) });
          externalAcademic = classes.map((externalClass) => {
            ...
            const result = calculateCourseGrade({ hasUnits: externalClass.hasUnits, unitCount: externalClass.unitCount, gradingScope: externalClass.gradingScope, passingAverage: externalClass.passingAverage, unitPassingAverages: externalClass.unitPassingAverages }, grades.map((grade) => ({ score: normalizeExternalGrade(grade.score, grade.maxScore), unit: grade.unitNumber })));
            const attendanceSummary = summarizeStudentAttendance(attendanceRows.filter((record) => record.externalClassId === externalClass.id), records[0]?.id || 0, externalClass.startDate ? new Date(externalClass.startDate).toISOString().slice(0, 10) : null, externalClass.endDate ? new Date(externalClass.endDate).toISOString().slice(0, 10) : null);
            return { className: externalClass.className, courseName: externalClass.courseName, hasUnits: Boolean(externalClass.hasUnits), unitCount: result.units.length || 1, gradingScope: result.scope, passingAverage: result.passingAverage, status: getAcademicStatus(result, attendanceSummary, externalClass.maxAbsencePercent), attendanceRate: attendanceSummary.attendanceRate, absent: attendanceSummary.absent, totalClasses: attendanceSummary.totalClasses, gradeCount: grades.length, units: result.units.map((unit) => ({ number: unit.unit, average: unit.average, minimum: unit.passingAverage, ratio: unit.average === null ? 0 : Math.min(100, (unit.average / 10) * 100) })) };
          });
        }
      }
    } catch (error) {
