...
import { Download, FileText, Loader2, Search, ShieldAlert, Trash2 } from "lucide-react";
...
export default function NotesPage() {
  ...
  useEffect(() => {
    fetch("/api/notes")
      ...
      .catch((error) => toast.error(error instanceof Error ? error.message : "Erro ao carregar anotações."))
      .finally(() => setLoading(false));
  }, []);

  const filtered = useMemo(
    () => notes.filter((note) => `${note.lesson?.title || ""} ${note.note}`.toLowerCase().includes(query.toLowerCase())),
    [notes, query],
  );
  ...
  async function save(note: Note) {
    ...
    try {
      const response = await fetch("/api/notes", {
        ...
        body: JSON.stringify({ lessonId: note.lessonId, note: note.note }),
      });
  ...
  async function remove(note: Note) {
    ...
    try {
      const response = await fetch(`/api/notes?lessonId=${note.lessonId}`, { method: "DELETE" });
      const data = await response.json().catch(() => null);
      ...
      toast.error(error instanceof Error ? error.message : "Não foi possível excluir a anotação.");
    } finally {
      setDeletingId(null);
    }
  }
  ...
  function exportNotes() {
    const content = filtered
      .map(
        (note) => `Aula: ${note.lesson?.title || `#${note.lessonId}`}\nAtualizada em: ${new Date(note.updatedAt).toLocaleString("pt-BR")}\n\n${note.deletedByAdminAt ? "(Excluída por um administrador)" : note.note}\n\n---\n`,
      )
      .join("\n");
    const blob = new Blob([content || "Nenhuma anotação encontrada."], { type: "text/plain;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = "minhas-anotacoes.txt";
    anchor.click();
  ...
  return (
    <>
              ...
              <article key={note.id} className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                ...
                {note.deletedByAdminAt ? (
                  ...
                  <>
                    ...
                    <button onClick={() => void save(note)} disabled={savingId === note.id} className="mt-3 inline-flex items-center gap-2 rounded-xl bg-red-600 px-4 py-2 text-xs font-bold text-white hover:bg-red-700 disabled:opacity-60">{savingId === note.id && <Loader2 className="animate-spin" size={14} />} Salvar alteração</button>
                  </>
                )}
              </article>
            ))}
      ...
      <ConfirmDialog
        ...
        description="Esta ação removerá a anotação da sua conta. O progresso da aula não será alterado."
