        ...
        {filteredItems.length === 0 ? (
          ...
          filteredItems.map((course) => (
            <div key={course.id} className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm flex flex-col justify-between group">
              ...
              <div className="p-6 pt-0 flex items-center justify-between border-t border-gray-100 mt-4 gap-2">
                <span className="font-extrabold text-base text-gray-900">{course.price}</span>
                <div className="flex items-center gap-2">
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => handleRemove(course.id)}
                    className="border-gray-300 text-gray-600 hover:text-red-600 hover:border-red-300 font-semibold rounded-xl text-xs"
                  >
                    Remover
                  </Button>
                  <Link href={`/cursos/${course.id}`}>
                    <Button size="sm" className="bg-red-600 hover:bg-red-700 text-white font-semibold rounded-xl text-xs gap-1">
                      Comprar Agora <ShoppingCart size={14} />
                    </Button>
                  </Link>
                </div>
              </div>
