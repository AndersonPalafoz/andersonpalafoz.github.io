...
**Próximo passo exato:** adicionar controles de edição/exclusão de presença ao detalhe da turma e validar os fluxos com uma sessão autorizada. Os controles de sessão já estão conectados ao detalhe: edição e exclusão enviam `id + offerId`, exigem confirmação, preservam o escopo da turma e atualizam o resumo acadêmico sem reload. A base de presença já filtra por `offerId` e grava o vínculo na criação. TypeScript, lint focalizado, testes acadêmicos e `git diff --check` passaram neste ciclo. A migration já foi aplicada no Neon principal e está registrada localmente como `0089_link_sessions_to_offers.sql`.
