...
**Próximo passo exato:** conectar os controles visuais de edição/exclusão de presença ao detalhe da turma e validar os fluxos com uma sessão autorizada. A API de presença agora oferece `PATCH` e `DELETE` protegidos por `attendanceId + sessionId + offerId`, verificando primeiro que a sessão pertence à turma; TypeScript, lint focalizado e `git diff --check` passaram neste ciclo. Os controles de sessão já estão conectados ao detalhe: edição e exclusão enviam `id + offerId`, exigem confirmação, preservam o escopo da turma e atualizam o resumo acadêmico sem reload. A migration já foi aplicada no Neon principal e está registrada localmente como `0089_link_sessions_to_offers.sql`.
