...
A expansão gradual do azul nas áreas públicas e autenticadas foi revertida a pedido do usuário. O novo plano incremental foi iniciado: o azul será introduzido como detalhe de linha, divisor ou sombra, com revisão visual entre etapas. A primeira aplicação desta fase reforça apenas a sombra inferior/direita dos três cards de benefícios da homepage por meio de `.public-accent-card`. A opção aprovada foi implementada no primeiro card: ele agora funciona como destaque de cursos, com etiqueta “Próxima aula”, ícone azul suave e CTA azul “Conhecer os cursos”, sem o detalhe circular decorativo do canto superior direito; os outros dois cards permanecem neutros, sem `.public-accent-card`. A paleta original foi restaurada em todas as áreas; permanece a linha azul fina antes do rodapé, explicitamente aprovada. Em 2026-09-10, a linha foi ajustada para `#2563EB`, um azul vivo de linguagem educacional e comunicação, alinhado à referência visual fornecida pelo usuário. Nesta etapa, a mesma linguagem de linha fina foi aplicada apenas como divisor entre hero e conteúdo em `/cursos`, `/materiais` e `/blog`; nenhuma cor de fundo, CTA ou regra de negócio foi alterada. Em seguida, as caixas `.surface-card` receberam uma sombra azul deslocada para a direita e para baixo, seguindo a referência visual; a borda lateral permanece neutra e o hover reforça levemente a sombra. Tabelas de dados e estados vazios mantêm o contorno anterior em intensidade menor, sem azul nos CTAs e controles.
