...
**Próximo passo exato:** validar os fluxos de presença e atividades com uma sessão autorizada em desktop e mobile. A aba Presença lista os registros por aluno e sessão, permite alterar entre presente/ausente/justificada e excluir com confirmação, usando `attendanceId + sessionId + offerId`; a aba Atividades permite editar título, descrição e prazo, além de excluir com confirmação, sempre atualizando o resumo acadêmico sem reload. Neste ciclo, a main remota foi reconciliada sem apagar dados; o polling de notificações recebeu cancelamento seguro e a contagem global ficou em 56 erros, 40 avisos e 38 ocorrências de `react-hooks/set-state-in-effect`. Testes focados (6 testes), TypeScript, `git diff --check` e build passaram. Sem sessão, o navegador confirmou corretamente o redirecionamento para login em 384×591. O próximo checkpoint deve sincronizar este estado com a branch `main`.
